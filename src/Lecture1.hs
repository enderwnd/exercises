...
subString start end str
    | null str = ""
    | (end < 0) || (end < start) = ""
    | otherwise = let startPos = max start 0 in take (end - startPos + 1) (drop startPos str)
...
strSum str = let numbers = words str in addNums 0 numbers
    where
        ...
        addNums result arrNums
            | null arrNums = result
            | otherwise = result + addNums (read (head arrNums)) (tail arrNums) 
...
lowerAndGreater n list =  show n ++ " is greater than " ++ show (checkHead (\x y -> x > y) 0 n list) ++ " elements and lower than " ++ show (checkHead (\x y -> x < y) 0 n list) ++ " elements"
    where
        ...
        checkHead satisfies result comp searchList
            | null searchList = result
            | satisfies comp (head searchList) = checkHead satisfies (result + 1) comp (tail searchList)
            | otherwise = checkHead satisfies result comp (tail searchList)
